    ...
    */
    ...
    return function (inputConfig) {
        ...
        var me = this,
            ...
            // Board components
            elmContainer, elmPlayingField, elmAboutPanel, elmLengthPanel, elmHighscorePanel, elmWelcome, elmTryAgain,
            elmWin, elmPauseScreen;
        ...
        function createBoardElements() {
            ...
            elmAboutPanel.innerHTML = "<a href='https://github.com/patorjk/JavaScript-Snake' class='snake-link'>source code</a> ";
...
function printGameArea(playingBoard, snake) {
    let board = generateBoardArray(playingBoard, snake);
    let boardBase64 = bytesArrayToBase64(board);
    ...
    stompClient.send(wsEndpoints.update, {}, JSON.stringify({'pixelsBrightnesses': boardBase64}));
    lastWsTime = Date.now();

}

function generateBoardArray(playingBoard, snake) {
    var field = [];
    for (var y = 0; y < playingBoard.grid.length; y++) {
        field.push([]);
        for (var x = 0; x < playingBoard.grid[y].length; x++) {
            if (playingBoard.grid[y][x] != 0) {
                field[y].push(255);
            } else {
                field[y].push(0);
            }
    ...
    console.log(playingBoard.getFoodElement);
    return field;
}
